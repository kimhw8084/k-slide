...
Loop on `kslide_next` until `VERIFIED`, `NEEDS_REVIEW`, or `COMPLETE`. Process each `READY`/`REPAIR_READY` work unit through `kslide_evidence`, read its context image and required risk crops, and structured `kslide_submit`. Verify the whole run when all units are translated, then finalize on `VERIFIED`; repair exact targets only. Prefer explicit unresolved output over guessing.
