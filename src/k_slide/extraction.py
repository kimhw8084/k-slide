...
def _tables(unit: Any, native_items: list[dict[str, Any]]) -> tuple[tuple[EvidenceTable, ...], list[dict[str, Any]]]:
    ...
    for item in native_items:
        ...
        if isinstance(unit_value, (list, tuple)):
            unit_text = str(unit_value[0]).strip() if len(unit_value) == 1 and str(unit_value[0]).strip() else None
        else:
        ...
        for cell in raw_cells:
            ...
            cell_facts = extract_numeric_facts(raw_text, source_object_id=cell_id, source_table_id=table_id, source_cell_id=cell_id)
