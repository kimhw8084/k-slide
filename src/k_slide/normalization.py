...
def _derive_table_unit(table_value: dict[str, Any], cells: list[dict[str, Any]]) -> str | None:
    """Derive one unit only from explicit table header/source-note evidence."""
...
def _connector_endpoints(shape: Any, source_ids_by_shape_id: dict[int, str]) -> dict[str, Any] | None:
    try:
        ...
        def arrow_type(name: str) -> str:
            ...
            if line is None:
                return "none"
            end = next((item for item in line if item.tag.rsplit("}", 1)[-1] == name), None)
            if end is None:
                return "none"
            value = str(end.get("type") or "none").strip().lower()
            return value or "none"

        start_arrow_type = arrow_type("headEnd")
        end_arrow_type = arrow_type("tailEnd")
        has_start_arrow = start_arrow_type != "none"
        has_end_arrow = end_arrow_type != "none"
    ...
    for slide_index, slide in enumerate(presentation.slides):
        ...
        for shape_index, shape in shape_records:
            ...
            if getattr(shape, "has_table", False):
                ...
                unit_label = _derive_table_unit(item["table"], cells)
                if unit_label is not None:
                    item["table"]["unit"] = unit_label
