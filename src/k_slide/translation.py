...
def _source_fact_connector_relation(relation: dict[str, Any], evidence: EvidenceIR) -> bool:
    ...
    for connector_id in source_ids:
        ...
        if direction_evidence not in {"start_to_end", "end_to_start", "bidirectional"}:
            # stCxn/endCxn prove connectivity only.  Geometry is never an
            # arrow-direction substitute.
            continue
...
def _chart_point(chart: dict[str, Any], claim: dict[str, Any], series: dict[str, Any]) -> dict[str, Any]:
    ...
    point = next((item for item in points if item.get("point_index") == point_index), None)
    if point is None:
...
def _validate_chart_claim(claim: Any, charts: list[dict[str, Any]]) -> None:
    ...
    kind = claim.get("kind")
    if not isinstance(chart_id, str) or not isinstance(kind, str) or kind not in {"trend", "value", "ranking", "comparison"}:
    ...
    if kind == "ranking":
        ...
        rank = claim.get("rank")
        if ranking not in {"highest", "lowest"} or isinstance(rank, bool) or not isinstance(rank, int) or rank < 1:
        ...
        for item in chart.get("series", []):
            if not isinstance(item, dict):
                continue
            other_point = _chart_point(chart, {**claim, "series_index": item.get("series_index"), "series_name": item.get("name")}, item)
            values.append((float(other_point["value"]), int(item["series_index"])))
        ...
        values.sort(reverse=ranking == "highest")
        expected = values[rank - 1][1] if rank <= len(values) else None
    ...
    other_point = _chart_point(chart, claim, other)
    left = float(point["value"])
    right = float(other_point["value"])
    ...
    for chart_element in charts:
        ...
        for series in series_list:
            ...
            for window in series_clauses:
                ...
                if actual is not None and ((observed[0] == "up" and actual != "increasing") or (observed[0] == "down" and actual != "decreasing") or (observed[0] == "flat" and actual != "flat")):
...
class TranslationPatch:
    ...
    def validate_against(self, evidence: EvidenceIR) -> None:
        ...
        for patch in self.regions:
            ...
            mismatch = modality_mismatch(source_text, patch.commitment_status, patch.speech_act, language_bound=source.language is not None)
            if mismatch:
                raise KSlideError(ErrorCode.MODALITY_MISMATCH, mismatch, {"region_id": patch.region_id})
            english_mismatch = english_modality_mismatch(source_text, patch.english, patch.commitment_status, language_bound=source.language is not None)
            if english_mismatch:
        ...
        for relation in self.visual_interpretations:
            ...
            if chart_claim is not None:
                if not isinstance(chart_claim, dict) or not charts or chart_claim.get("chart_element_id") not in source_element_ids or chart_claim.get("chart_element_id") not in evidence_ids:
