...
and published through its approved private transport.
"""
...
def build(*, output: Path, subject_git_sha: str, candidate_profile: Path, production_dependency_lock: Path, termbase_overlay: Path | None = None, ocr_asset_manifest: Path | None = None) -> dict[str, Any]:
    if len(subject_git_sha) != 40 or set(subject_git_sha.lower()) - set("0123456789abcdef"):
        raise EvidenceValidationError("bundle subject SHA is invalid")
    ...
    if candidate.get("subject_git_sha") not in {None, "", "UNSET", subject_git_sha}:
    ...
    manifest = {"schema_version": "1.0", "subject_git_sha": subject_git_sha, "files": entries}
    atomic_write_json(output / "certification-bundle.json", manifest, mode=0o600)
    return {"status": "PASS", "subject_git_sha": subject_git_sha, "file_count": len(entries)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = build(output=args.output, subject_git_sha=args.subject_sha, candidate_profile=args.candidate_profile, production_dependency_lock=args.production_dependency_lock, termbase_overlay=args.termbase_overlay, ocr_asset_manifest=args.ocr_asset_manifest)
