...
def main(argv: list[str] | None = None) -> int:
    ...
    _run_check("representative_engine_diagnostics", ["docker", "run", "--rm", "--network", "none", "-v", engine_mount, "-v", candidate_mount, "--entrypoint", "python", args.image, "-m", "evals.run_engine_eval", "--output", "/out/diagnostics-table", "--candidate-profile", "/candidate.json", "--subject-sha", subject_sha, "--split", "development", "--scenario-ids", "scenario-0031", "--formats", "png", "pdf", "pptx", "--ocr-provider", "paddle", "--fail-on-critical"], diagnostic_dir / "engine-command.json", outcomes, expected_after=diagnostic_dir / "summary.json")
