...
- KSA-16 authoritative classification admission: host/platform classification travels independently with every host input, defaults only missing labels to `company_confidential`, and is evaluated exactly against a deployment-controlled route/data-use policy before source validation, immutable snapshots, normalization, rendering, crops, or EvidenceIR. Denied, unknown, malformed, unresolved, route-mismatched, or policy-drifted inputs fail as source-free operational admission failures; OpenCode tool/model values cannot override trusted host metadata. Route and exact policy identities bind candidate fingerprints and resumable run environments. The reference policy is a deterministic test adapter only; company policy approval, model-data attestation, target-Gemma qualification, and production certification remain external gates. See [ADR 0044](docs/adr/0044-classification-admission.md).
...
KSA-16 repository-side verification: `PYTHONPATH=src:. python -m unittest tests.test_chg16_classification_admission -q` PASS (7 tests); focused KSA-14/15, host, environment, retention, storage, certification, and runtime regressions PASS (197 tests, 1 optional skip); full repository suite PASS (334 tests, 3 optional skips); Python 3.11/3.12 compile checks PASS; `git diff --check` PASS. The decisive matrix covers exact allowed/denied/unknown labels, the missing-label default, malformed and unresolved policy, route and policy identity mismatch, mixed multi-file admission, OpenCode classification spoofing, exact EvidenceIR preservation, candidate fingerprint drift, resume/environment incompatibility, and source-free failure persistence. OpenCode loader compatibility PASS; the full TypeScript fixture remains blocked by the pre-existing CLI success fixture's missing KSA-10 candidate/runtime subjects, and Bun/standalone `tsc` are unavailable in this host. No company policy approval, Gemma inference, model-data attestation, target-Gemma certification, BUILD COMPLETE, release, or production certification is claimed.
