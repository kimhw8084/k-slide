...
KSA-09 repository-side verification: `PYTHONPATH=src:. python -m unittest tests.test_chg16_scoped_admission -q` PASS (8 tests); the tests exercise multi-process scoped admission/claims, one-active-heavy-run FIFO overflow, independent scopes, restart recovery, terminal/cancellation promotion, recreated-store CAS/idempotency, wrong-scope/guessed-ID denial, and source-free isolated namespaces. Live company storage/job-service qualification remains an external gate.
