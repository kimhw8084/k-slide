...
class KSA24F24RepairTests(unittest.TestCase):
    def test_connector_direction_requires_arrow_evidence_and_supports_start_arrow_reverse(self) -> None:
        ...
        def relation(edge: str, endpoints: tuple[str, str], direction: str) -> dict:
            return {"relation_id": edge, "interpretation": "flow", "evidence_ids": [edge], "source_element_ids": [*endpoints, edge], "relation_type": "next", "direction": direction, "provenance": "source_fact"}
    ...
    def test_chart_claims_bind_exact_series_points_and_categories(self) -> None:
        ...
        for bad in (claim(series_index=1), claim(direction="decreasing"), claim(series_name="South")):
            with self.assertRaises(KSlideError):
        ...
        parse_translation_patch(_patch(evidence, relation={**good, "chart_claim": value_claim})).validate_against(evidence)
        for bad in ({**value_claim, "value": -1234.4}, {**value_claim, "category": "Q2"}):
            with self.assertRaises(KSlideError):
                parse_translation_patch(_patch(evidence, relation={**good, "chart_claim": bad})).validate_against(evidence)
        parse_translation_patch(_patch(evidence, relation={**good, "chart_claim": claim(series_index=2, series_name="Flat", direction="flat")})).validate_against(evidence)
        with self.assertRaises(KSlideError):
            parse_translation_patch(_patch(evidence, relation={**good, "chart_claim": claim(series_index=3, series_name="Nullable", direction="increasing")})).validate_against(evidence)
        ...
        parse_translation_patch(_patch(evidence, relation={"relation_id": "number", "interpretation": "The value is -1,234.5%", "evidence_ids": ["fixture-chart"], "source_element_ids": ["fixture-chart"], "provenance": "supported_interpretation"})).validate_against(evidence)
        with self.assertRaises(KSlideError):
            parse_translation_patch(_patch(evidence, relation={"relation_id": "number", "interpretation": "The value is -1,234.4%", "evidence_ids": ["fixture-chart"], "source_element_ids": ["fixture-chart"], "provenance": "supported_interpretation"})).validate_against(evidence)

    def test_modality_requires_metadata_and_rejects_english_strengthening(self) -> None:
        cases = (("결정", "decided", "decision", "The decision is approved."), ("약속", "committed", "status", "The work is committed."), ("계획", "planned", "status", "The work is planned."), ("예정", "scheduled", "status", "The work is scheduled."), ("목표", "target", "status", "The target is the target."), ("제안", "proposed", "status", "The proposal is proposed."), ("검토 중", "under_review", "status", "The work is under review."), ("가능", "possible", "status", "The work is possible."), ("잠정", "tentative", "status", "The result is tentative."), ("전망", "forecast", "status", "The forecast is projected."), ("완료", "completed", "status", "The work is completed."), ("진행 중", "in_progress", "status", "The work is in progress."))
    ...
    def test_table_unit_scope_blank_and_merge_state_are_engine_owned(self) -> None:
        cells = [{"cell_id": "t-r0-c0", "row": 0, "column": 0, "text": "KRW 억", "is_header": True}, {"cell_id": "t-r0-c1", "row": 0, "column": 1, "text": "   ", "is_header": True}, {"cell_id": "t-r1-c0", "row": 1, "column": 0, "text": "5", "is_header": False}, {"cell_id": "t-r1-c1", "row": 1, "column": 1, "text": "\t", "is_header": False}, {"cell_id": "t-r2-c0", "row": 2, "column": 0, "text": None, "is_spanned": True, "is_merge_origin": False}]
        ...
        self.assertIsNone(facts[0].get("source_unit"))
