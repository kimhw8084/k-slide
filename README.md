...
Administrators can run `k-slide doctor --production` against a managed deployment profile. It fails closed until the approved model, OCR assets/provider, OpenCode runtime, both centrally supplied retention policies (content and operational metadata), tenant isolation, egress policy, and certification fingerprint are proven. Retention policy values are deployment inputs; K-Slide has no product-default duration. The non-model-facing `k-slide retention-cleanup` command reads the resolved canonical `retention_policy` from the managed production profile and uses only `content_retention_days` for terminal user-content cleanup; it does not accept an ad-hoc duration. The `k-slide support-bundle` command is intended for an administrator or deployment wrapper; support bundles contain operational metadata only and exclude source content.

KSA-34 candidate-bound rollout policy gates new admissions at the shared engine boundary used by the CLI and OpenCode `/k-slide` tool. Candidate deployments require a process-configured deployment authority with named cohort membership, signed versioned policy/state, explicit stages, and a monotonic state revision. Disable blocks new runs immediately; rollback selects only a previously authorized candidate and leaves admission closed until that runtime is active. Existing admitted runs retain their bound environment identity. Rollout state is source-free and does not change champion, governance, release, or certification state. See [`docs/ksa34-rollout-controls.md`](docs/ksa34-rollout-controls.md).

The ordinary support path is always source-free. The platform-neutral controlled content-support API is a separate exceptional path: it requires an externally supplied, exact user/workspace/scope/run authorization decision with approved purpose, role/capability, approval evidence, and finite expiry; it selects only typed artifact references and records a source-free central audit. K-Slide does not implement company IAM, support-role membership, business approval, credential storage, or content transfer. A live company authority and any approved support transfer mechanism remain external deployment qualification gates; the standalone CLI does not manufacture or expose content-bearing support authorization.
