...
monotonic FIFO sequence. Terminal `DONE`, processing-failed, or acknowledged
canceled jobs release the slot and promote at most one next eligible entry.
KSA-06 compare-and-set and operation-marker idempotency remain authoritative
for checkpoint and result commits. A bound `ScopedPaaSRunStore` exposes only
the authorized job's store view to a worker.
...
- Independent scopes use independent locks and namespaces and can progress
  concurrently.
- Recreated reference services recover queue order from durable admission
  sequence and job records; they do not scan one globally shared active queue.
